    ...
    public class BlogRepository : IBlogRepository
    {
       ...
       CancellationToken cancellationToken = default)
        ...
        private IQueryable<Post> FilterPosts(PostQuery condition)
        {
            ...
            condition.GetTagListAsync();
            if (condition.SelectedTag != null && condition.SelectedTag.Count() > 0)
            {
                posts = posts.Where(p => condition.SelectedTag.Intersect(p.Tags.Select(t => t.Name)).Count() > 0);
            }
            return posts;
